...
class Reservation {
	...
	public function ShowReservationInfo()
	{
		...
		if(count($this->arrReservation) > 0)
		{
			...
			foreach($this->arrReservation as $key => $val)
			{
				...
				if($result[1] > 0){			
					...
					echo '<tr>
							...
							<td align="'.$class_right.'">'.Currencies::PriceFormat($room_price_w_meal_extrabeds * $this->currencyRate, '', '', $this->currencyFormat).'&nbsp;<a class="price_link" href="javascript:void(0);" onclick="javascript:appToggleElement(\'row_prices_'.$key.'\')" title="'._CLICK_TO_SEE_PRICES.'">(+)</a></td>
						</tr>
			...
			if($this->discountCampaignID != '' || $this->discountCoupon != ''){
				echo '<tr>
						<td colspan="7"></td>
						<td class="td '.$class_left.'" colspan="3"><b><span style="color:#a60000">'._DISCOUNT.': ('.Currencies::PriceFormat($this->discountPercent, '%', 'after', $this->currencyFormat).')</span></b></td>
		...
		if(count($this->arrReservation) > 0){
			...
			foreach($this->arrReservation as $key => $val){
				...
				if($result[1] > 0){
					...
					echo '<tr>
							...
							<td align="'.$class_right.'">'.Currencies::PriceFormat($room_price_w_meal_extrabeds * $this->currencyRate, '', '', $this->currencyFormat).'&nbsp;</td>
						</tr>
			...
			if($extras[1]){
				...
				for($i=0; $i<$extras[1]; $i++){
					...
					echo '<td>&nbsp;</td>';
					echo '<td>'.draw_numbers_select_field('extras_'.$extras[0][$i]['id'], $extras_id, '0', $extras[0][$i]['maximum_count'], 1, 'extras_ddl', 'onchange="appUpdateTotalSum('.$i.',this.value,'.(int)$extras[1].')"', false).'</td>';
					echo '</tr>';
				}
				echo '</table></td></tr>';								
			}
			...
			echo '<table border="0" width="100%">';
				...
				echo '<tr>';
					if($pre_payment_type == 'first night' && $this->first_night_possible){
						...
						echo '<td>';
						echo '<input type="radio" name="pre_payment_type" id="pre_payment_fully" value="full price"'.(($pre_payment_type_post == 'full price') ? ' checked="checked"' : '').' /> <label for="pre_payment_fully">'._FULL_PRICE.'</label> &nbsp;&nbsp;';
						echo '<input type="radio" name="pre_payment_type" id="pre_payment_partially" value="first night"'.(($pre_payment_type_post == 'first night') ? ' checked="checked"' : '').' /> <label for="pre_payment_partially">'._FIRST_NIGHT.'</label>';
						...
						echo '<td>';
						echo '<input type="radio" name="pre_payment_type" id="pre_payment_fully" value="full price"'.(($pre_payment_type_post == 'full price') ? ' checked="checked"' : '').' /> <label for="pre_payment_fully">'._FULL_PRICE.'</label> &nbsp;&nbsp;';
						echo '<input type="radio" name="pre_payment_type" id="pre_payment_partially" value="percentage"'.(($pre_payment_type_post == 'percentage') ? ' checked="checked"' : '').' /> <label for="pre_payment_partially">'._PRE_PAYMENT.' ('.Currencies::PriceFormat($pre_payment_value, '%', 'after', $this->currencyFormat).')</label>';
						echo draw_hidden_field('pre_payment_value', $pre_payment_value, false, 'pre_payment_full');
					}else if($pre_payment_type == 'fixed sum' && $pre_payment_value > '0'){
						echo '<td>'._PAYMENT_METHOD.': </td>';
						echo '<td>';
						echo '<input type="radio" name="pre_payment_type" id="pre_payment_fully" value="full price"'.(($pre_payment_type_post == 'full price') ? ' checked="checked"' : '').' /> <label for="pre_payment_fully">'._FULL_PRICE.'</label> &nbsp;&nbsp;';
						echo '<input type="radio" name="pre_payment_type" id="pre_payment_partially" value="fixed sum"'.(($pre_payment_type_post == 'fixed sum') ? ' checked="checked"' : '').' /> <label for="pre_payment_partially">'._PRE_PAYMENT.' ('.Currencies::PriceFormat($pre_payment_value * $this->currencyRate, '', '', $this->currencyFormat).')</label>';
						echo draw_hidden_field('pre_payment_value', $pre_payment_value, false, 'pre_payment_full');
					}else{
						echo '<td colspan="2">';
						// full price payment
			...
			if($payment_types_defined){			
				...
				if(!empty($this->discountCoupon)){				
					...
					echo '<input type="button" class="button" id="discount_button" value="'._REMOVE.'" onclick="appSubmitCoupon(\'remove_coupon\')" />';
				}else{				
					...
					echo '<input type="button" class="button" id="discount_button" value="'._APPLY.'" onclick="appSubmitCoupon(\'apply_coupon\')" />';
				}
				echo '</td>';
				echo '</tr>';					
	...
	public function DrawReservation($payment_type, $additional_info, $extras = array(), $pre_payment_type = '', $pre_payment_value = '', $draw = true)
	{
		...
		if($result[1] > 0){
			...
			if($discount_value != ''){
				$fisrt_part .= 	'<tr><td>'._DISCOUNT.' </td><td> : </td><td> - '.Currencies::PriceFormat($discount_value, '', '', $this->currencyFormat).' ('.Currencies::PriceFormat($this->discountPercent, '%', 'after', $this->currencyFormat).')</td></tr>';
	...
	public function PlaceBooking($additional_info = '', $cc_params = array())
	{
		...
		$sql = 'SELECT id, booking_number, payment_type
				FROM '.TABLE_BOOKINGS.'
				WHERE customer_id = '.(int)$this->currentCustomerID.' AND
		...
		if($result[1] > 0){
			$booking_number = $result[0]['booking_number'];
			$status = ($result[0]['payment_type'] == 0 || $result[0]['payment_type'] == 5) ? 1 : 2;
			...
			if(in_array($payment_type, array('poa', 'online', 'paypal', '2co', 'authorize.net', 'bank.transfer'))){
				...
				if(database_void_query($sql)){					
					...
					$sql = 'INSERT INTO '.TABLE_BOOKINGS_ROOMS.'
								(id, booking_number, hotel_id, room_id, room_numbers, checkin, checkout, adults, children, rooms, price, extra_beds, extra_beds_charge, meal_plan_id, meal_plan_price)
							VALUES ';
					$items_count = 0;
					foreach($this->arrReservation as $key => $val){					
						$sql .= ($items_count++ > 0) ? ',' : '';
						$sql .= '(NULL, \''.$booking_number.'\', '.(int)$val['hotel_id'].', '.(int)$key.', \'\', \''.$val['from_date'].'\', \''.$val['to_date'].'\', \''.$val['adults'].'\', \''.$val['children'].'\', '.(int)$val['rooms'].', '.($val['price'] * $this->currencyRate).', '.(int)$val['extra_beds'].', '.($val['extra_beds_charge'] * $this->currencyRate).', '.(int)$val['meal_plan_id'].', '.($val['meal_plan_price'] * $this->currencyRate).')'; 
					}
					if(database_void_query($sql)){
						$booking_placed = true;						
					}else{
	...
	{		
		...
		if($result[1] > 0){
			...
			$booking_details .= (($result[0]['campaign_name'] != '') ? _DISCOUNT.': - '.Currencies::PriceFormat($result[0]['discount_fee'], $result[0]['symbol'], $result[0]['symbol_placement'], $this->currencyFormat).' ('.Currencies::PriceFormat($result[0]['discount_percent'], '%', 'after', $this->currencyFormat).' - '.$result[0]['campaign_name'].')<br />' : '');
			$booking_details .= (($result[0]['coupon_code'] != '') ? _DISCOUNT.': - '.Currencies::PriceFormat($result[0]['discount_fee'], $result[0]['symbol'], $result[0]['symbol_placement'], $this->currencyFormat).' ('.Currencies::PriceFormat($result[0]['discount_percent'], '%', 'after', $this->currencyFormat).' - '._COUPON_CODE.': '.$result[0]['coupon_code'].')<br />' : '');
			...
			if($result[1] > 0){
				...
				$booking_details .= (($allow_extra_beds == 'yes') ? '<th>'._EXTRA_BEDS.'</th>' : '');
				...
				for($i=0; $i < $result[1]; $i++){
					...
					$booking_details .= (($allow_extra_beds == 'yes' && !empty($result[0][$i]['extra_beds'])) ? '<td>'.$result[0][$i]['extra_beds'].' ('.Currencies::PriceFormat($result[0][$i]['extra_beds_charge'], $result[0][$i]['symbol'], '', $this->currencyFormat).')</td>' : '<td>0</td>');
					...
					$booking_details .= '<td align="right">'.Currencies::PriceFormat(($result[0][$i]['price'] + $result[0][$i]['meal_plan_price'] + $result[0][$i]['extra_beds_charge']), $result[0][$i]['symbol'], '', $this->currencyFormat).'</td>';
					$booking_details .= '</tr>';	
				}
