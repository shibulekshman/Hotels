...
class Bookings extends MicroGrid {
	...
	function __construct($user_id = '') {
		...
		if($objLogin->IsLoggedInAs('hotelowner')){
			...
			$this->arrFilteringFields[_ROOMS] = array('table'=>TABLE_BOOKINGS_ROOMS, 'field'=>'room_id', 'type'=>'dropdownlist', 'source'=>$arr_rooms, 'sign'=>'=', 'width'=>'', 'visible'=>(($this->user_id != '') ? false : true));
		}
		
		if($this->user_id == '') $this->arrFilteringFields[_CUSTOMER] = array('table'=>TABLE_CUSTOMERS, 'field'=>'last_name', 'type'=>'text', 'sign'=>'like%', 'width'=>'90px');
		$this->arrFilteringFields[_STATUS] = array('table'=>$this->tableName, 'field'=>'status', 'type'=>'dropdownlist', 'source'=>$arr_statuses_filter, 'sign'=>'=', 'width'=>'120px');
		$this->arrFilteringFields[_CHECK_IN] = array('table'=>TABLE_BOOKINGS_ROOMS, 'field'=>'checkin', 'type'=>'calendar', 'date_format'=>$date_format_settings, 'sign'=>'like%', 'width'=>'82px', 'visible'=>true);
		$this->arrFilteringFields[_CHECK_OUT] = array('table'=>TABLE_BOOKINGS_ROOMS, 'field'=>'checkout', 'type'=>'calendar', 'date_format'=>$date_format_settings, 'sign'=>'like%', 'width'=>'82px', 'visible'=>true);
		...
		$this->VIEW_MODE_SQL = 'SELECT
								...
								IF('.$this->tableName.'.is_admin_reservation = 0, CONCAT("<a href=\"javascript:void(\'customer|view\')\" onclick=\"open_popup(\'popup.ajax.php\',\'customer\',\'",'.$this->tableName.'.customer_id,"\',\''.Application::Get('token').'\')\">", CONCAT('.TABLE_CUSTOMERS.'.last_name, " ", '.TABLE_CUSTOMERS.'.first_name), "</a>"), "{administrator}") as customer_name,
								...
								IF(
									'.$this->tableName.'.status = 2 AND ((SELECT DATEDIFF(br.checkin, \''.@date('Y-m-d').'\') FROM '.TABLE_BOOKINGS_ROOMS.' br WHERE br.checkin > \''.@date('Y-m-d').'\' AND br.booking_number = '.$this->tableName.'.booking_number ORDER BY br.checkin ASC LIMIT 0, 1) >= '.$this->customers_cancel_reservation.'),
		...
		if($this->user_id != ''){
			...
			$this->arrViewModeFields = array(
				'created_date_formated'   => array('title'=>_DATE_CREATED, 'type'=>'label', 'align'=>'left', 'width'=>'', 'height'=>'', 'nowrap'=>'nowrap', 'maxlength'=>''),
				'booking_number'    	  => array('title'=>_BOOKING_NUMBER, 'type'=>'label', 'align'=>'center', 'width'=>'', 'height'=>'', 'maxlength'=>''),
				'mod_checkin_checkout'    => array('title'=>_CHECK_IN.' - '._CHECK_OUT, 'type'=>'label', 'align'=>'center', 'width'=>'170px', 'height'=>'', 'maxlength'=>''),
				'mod_room_numbers'        => array('title'=>_ROOM_NUMBERS, 'type'=>'label', 'align'=>'center', 'width'=>'100px', 'height'=>'', 'maxlength'=>''),
				'payment_type'            => array('title'=>_METHOD, 'type'=>'enum',  'align'=>'center', 'width'=>'', 'sortable'=>true, 'nowrap'=>'', 'visible'=>'', 'source'=>$this->arr_payment_types),
	...
	public function CancelRecord($rid)
	{
		...
		if(!$objLogin->HasPrivileges('cancel_bookings')){
			return false;
		}
	...
	public function DrawBookingInvoice($rid, $text_only = false, $type = 'html', $draw = true)
	{
		...
		if($result[1] > 0){
			...
			if($type == 'pdf'){
				...
				$output_pdf['booking_status'] = strip_tags($this->statuses_vm[$result[0]['status']]);
				...
				if($result[0]['campaign_name'] != '') $output_pdf['discount'] = Currencies::PriceFormat($result[0]['discount_fee'], $result[0]['symbol'], $result[0]['symbol_placement'], $this->currencyFormat).' ('.Currencies::PriceFormat($result[0]['discount_percent'], '%', 'after', $this->currencyFormat).' - '.$result[0]['campaign_name'].')';
				else if($result[0]['coupon_code'] != '') $output_pdf['discount'] = Currencies::PriceFormat($result[0]['discount_fee'], $result[0]['symbol'], $result[0]['symbol_placement'], $this->currencyFormat).' ('.Currencies::PriceFormat($result[0]['discount_percent'], '%', 'after', $this->currencyFormat).' - '._COUPON_CODE.': '.$result[0]['coupon_code'].')';
			...
			$part .= '</td>
				...
				<tr>
					<td colspan="2">';						
						...
						else if($result[0]['coupon_code'] != '') $part .= '<tr><td>&nbsp;'._DISCOUNT.': </td><td>- '.Currencies::PriceFormat($result[0]['discount_fee'], $result[0]['symbol'], $result[0]['symbol_placement'], $this->currencyFormat).' ('.Currencies::PriceFormat($result[0]['discount_percent'], '%', 'after', $this->currencyFormat).' - '._COUPON_CODE.': '.$result[0]['coupon_code'].')</td></tr>';
	...
	public function DrawBookingDescription($rid, $mode = '', $draw = true)
	{
		...
		if($result[1] > 0){
			...
			if($result[0]['mod_have_to_pay'] != 0){
				$content .= '<tr><td style="color:#960000">'._PAYMENT_REQUIRED.': </td><td style="color:#960000">'.Currencies::PriceFormat($result[0]['mod_have_to_pay'], $result[0]['symbol'], $result[0]['symbol_placement'], $this->currencyFormat).'</td></tr>';
			}			
	...
	public function AfterDeleteRecord()
	{
		...
		if($objLogin->IsLoggedIn() && ($this->booking_status > 1) && ($this->booking_is_admin_reserv == '0')){
	...
	public function PrepareInvoiceDownload($rid)
	{
		...
		$pdf->Cell(0,10, _FOR_BOOKING.$data['booking_number'], 0, 0, 'L');    $pdf->Cell(0,10, _DATE_CREATED.': '.$data['created_date'], 0, 1, 'R');
		...
		// ----------------		
		$pdf->SetFont('DejaVu','',9);      /* Reset the font */
		$pdf->Cell(70,5, _STATUS.': ', 'L', 0, 'L');           $pdf->Cell(0,5, $data['booking_status'], 'R', 1, 'L');
		...
		$pdf->Cell(25,6, _ROOM_TYPE, 'BT', 0, 'L', true);
		$pdf->Cell(30,6, (($hotels_count > 1) ? _HOTEL : ''), 'BT', 0, 'L', true);
		$pdf->Cell(19,6, _CHECK_IN, 'BT', 0, 'L', true);
		$pdf->Cell(19,6, _CHECK_OUT, 'BT', 0, 'L', true);
		$pdf->Cell(10,6, _ROOMS, 'BT', 0, 'C', true);
		$pdf->Cell(18,6, '##', 'BT', 0, 'C', true);
		$pdf->Cell(10,6, _ADULT, 'BT', 0, 'C', true);
		$pdf->Cell(10,6, (($data_rooms[0]['children'] != '') ? _CHILD : ''), 'BT', 0, 'C', true);
		//$pdf->Cell(10,6, (($data_rooms[0]['extra_beds'] != '') ? _EXTRA_BEDS : ''), 'BT', 0, 'C', true);
		$pdf->Cell(37,6, _MEAL_PLANS, 'BT', 0, 'R', true);
		$pdf->Cell(15,6, _PRICE, 'BTR', 1, 'R', true);
		...
		for($i = 0; $i < count($data_rooms); $i++){
			...
			$pdf->Cell(4,6, ($i+1).'.', 'LT', 0, 'L');
			$pdf->Cell(25,6, $data_rooms[$i]['room_type'], 'T', 0, 'L');
			$pdf->Cell(30,6, (($hotels_count > 1) ? $data_rooms[$i]['hotel_name'] : ''), 'T', 0, 'L');
			$pdf->Cell(19,6, $data_rooms[$i]['checkin'], 'T', 0, 'L');
			$pdf->Cell(19,6, $data_rooms[$i]['checkout'], 'T', 0, 'L');
			$pdf->Cell(10,6, $data_rooms[$i]['rooms'], 'T', 0, 'C');
			$pdf->Cell(18,6, $data_rooms[$i]['room_numbers'], 'T', 0, 'C');
			$pdf->Cell(10,6, $data_rooms[$i]['adults'], 'T', 0, 'C');
			$pdf->Cell(10,6, (($data_rooms[$i]['children'] != '') ? $data_rooms[$i]['children'] : ''), 'T', 0, 'C');
			//$pdf->Cell(10,6, (($data_rooms[$i]['extra_beds'] != '') ? $data_rooms[$i]['extra_beds'].' ('.$data['booking_currency'].number_format($data_rooms[$i]['extra_beds_charge'], 2).')' : ''), 'T', 0, 'C');
			$pdf->Cell(37,6, $data_rooms[$i]['meal_plan_name'].' ('.$data['booking_currency'].number_format($data_rooms[$i]['meal_plan_price'], 2).')', 'T', 0, 'R');
			$pdf->Cell(15,6, $data_rooms[$i]['price'], 'TR', 1, 'R');
			$total_room_price += ($data_rooms[$i]['price_wo_currency'] + $data_rooms[$i]['meal_plan_price'] + $data_rooms[$i]['extra_beds_charge']);

			if($data_rooms[0]['extra_beds'] != ''){
				$pdf->Cell(4,6, '', 'LB', 0, 'L');
				$pdf->Cell(193,6, _EXTRA_BEDS.': '.$data_rooms[$i]['extra_beds'].' ('.$data['booking_currency'].number_format($data_rooms[$i]['extra_beds_charge'], 2).')', 'BR', 1, 'L');				
			}
		}
		$pdf->SetFont('DejaVu','',9);      /* Reset the font */
		...
		$this->message = _DOWNLOAD_INVOICE.' (PDF): <a href="javascript:void(\'pdf\')" onclick="javascript:appGoToPage(\'index.php?admin=export&file=invoice.pdf\');window.setTimeout(function(){location.reload()},2000);">'._FOR_BOOKING.$data['booking_number'].'</a>';
