...
if($checkin_year_month == '0' || $checkin_day == '0' || $checkout_year_month == '0' || $checkout_day == '0'){
	...
	if($check_partially_overlapping == 'yes'){
		$from_ipackage = strtotime($min_nights_packages['start_date']);
		$to_ipackage = strtotime($min_nights_packages['finish_date']);
		$ipackage = 0;	
		for($i = 0; $i < $nights; $i++){
			$ireservation_date = strtotime($checkin_year.'-'.$checkin_month.'-'.$checkin_day . '+'. $i .'day');
			if($from_ipackage <= $ireservation_date && $to_ipackage >= $ireservation_date){
				$ipackage = 1 + $ipackage;
			}
		}
	}
	...
	if($min_nights > $nights){
		draw_important_message(
			str_replace(array('_PACKAGE_NAME_', '_NIGHTS_', '_FROM_', '_TO_'),
						array($min_nights_packages['package_name'], '<b>'.$min_nights.'</b>', '<b>'.format_date($min_nights_packages['start_date']).'</b>', '<b>'.format_date($min_nights_packages['finish_date']).'</b>'),
		...
		draw_important_message(
			str_replace(array('_PACKAGE_NAME_', '_NIGHTS_', '_FROM_', '_TO_'),
						array($min_nights_packages['package_name'], '<b>'.$max_nights.'</b>', '<b>'.format_date($min_nights_packages['start_date']).'</b>', '<b>'.format_date($min_nights_packages['finish_date']).'</b>'),
						_MAXIMUM_NIGHTS_ALERT
			)
		);
    }else if($check_partially_overlapping == 'yes' && $min_nights_packages['minimum_nights'] > $ipackage){
